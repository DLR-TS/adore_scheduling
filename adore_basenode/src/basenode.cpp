...
namespace adore
{
    ...
    {
        /**
         * constructor - duration_between_function_calls is the reciprocal of the main frequency of the node.
         */
            ...
            : rclcpp::Node(node_name), m_use_scheduler(false)
        {
            ...
            this->declare_parameter(adore::scheduling::PARAM_NAME_USE_SCHEDULER, m_use_scheduler);
            this->get_parameter(adore::scheduling::PARAM_NAME_USE_SCHEDULER, m_use_scheduler);
            if (m_use_scheduler && !use_sim_time)
            {
                std::cout << std::endl
                          << "adore::scheduling::BaseNode: parameter use_sim_time for node " << node_name << "is False while parameter use_scheduler is set to True. If use_scheduler is set to True, use_sim_time must be set to True, too." << std::endl;
                throw std::logic_error("Parameter issue: use_scheduler is True while use_sim_time is not True.");
            }
        ...
        void BaseNode::init(std::chrono::nanoseconds duration_between_function_calls)
        {
            ...
            m_timer = rclcpp::create_timer(m_node, m_node->get_clock(), m_time_between_function_calls, std::bind(&BaseNode::mainCallback, this));

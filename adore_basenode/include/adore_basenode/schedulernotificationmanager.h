...
namespace adore_if_ros_scheduling
{
    ...
    class SchedulerNotificationManager
    {
      ...
      public:
        SchedulerNotificationManager(rclcpp::Node *node, unsigned int id, uint32_t duration, bool reg = true)
        : m_duration_in_nanoseconds(duration), m_NANOS_PER_SECOND(1e9)
        {
        ...
        uint32_t getNanosPerSecond() const
        {
            return m_NANOS_PER_SECOND;
        }
        void resume()
        {
            m_pause = false;
